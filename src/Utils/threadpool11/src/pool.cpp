...
#include "../include/threadpool11/pool.hpp"

#include <vector>

namespace threadpool11 {

typedef boost::lockfree::queue<Work::Callable*, boost::parameter::void_, boost::parameter::void_, boost::parameter::void_> queueT;

Pool::Pool(std::size_t worker_count)
    : worker_count_(0)
    , active_worker_count_(0)
    , are_all_really_finished_{true}
    , work_queue_(std::unique_ptr<queueT>(new queueT(0)))
    , work_queue_size_(0) {
